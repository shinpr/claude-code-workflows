...
2. From the matched files, also exclude every file matching any of these patterns — they are not implementation tasks owned by recipe-build's plan: `*-task-prep-*.md` (prep tasks, owned by recipe-prepare-implementation), `_overview-*.md` (overview file), `*-phase*-completion.md` (phase completion files), `review-fixes-*.md` (owned by recipe-review), `integration-tests-*-task-*.md` (owned by recipe-add-integration-tests)
