...
| Neither exists + Design Doc exists | No plan, no Consumed Task Set, but `docs/design/*.md` exists | Invoke work-planner to create a work plan, then run document-reviewer (`dev-workflows-frontend:document-reviewer`, doc_type: WorkPlan). Apply the Review Resolution Gate: update for `apply`, re-review with `prior_feedback`, progress when all actionable findings are `decline`, and escalate unresolved `user_decision_required`; then present the resolved plan for batch approval before task materialization |
