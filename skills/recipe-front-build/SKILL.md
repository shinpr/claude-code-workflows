...
- code-verifier (subagent_type: "dev-workflows-frontend:code-verifier") → resolved `doc_type`, `document_path`, and complete implementation `code_paths`
- security-reviewer (subagent_type: "dev-workflows-frontend:security-reviewer") → the same typed `governingDocuments` and complete `implementationFiles`
