...
成果物を生成しただけでは、ワークフローは次へ進みません。採用する設計を左右する前提は、承認前に実際に確認できる根拠で解消します。リポジトリ内の根拠や信頼できる情報源だけでは判断できない場合、検証工程で不足している根拠を特定します。その根拠を得る最小限の方法が動作確認なら、エージェントが範囲を絞って一度だけ実行します。作業計画は、実装を許可する前に、範囲、依存順序、実行可能な検証方法についてレビューされます。各タスクはそのタスクに絞ったチェックと該当するリポジトリチェックを通過してからコミットされます。段階的な実装が完了すると、設計との整合性、観測可能な動作のカバレッジ、セキュリティを別々にレビューします。
