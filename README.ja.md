...
実装範囲の承認後は、Claudeが各タスクに絞った検証、リポジトリの品質チェック、コミット、最終レビューまで進めます。通常の実装判断で逐一確認を求めることはありません。合意したプロダクト成果や対象外を変える必要がある場合だけユーザーに判断を求め、技術設計や実装上の選択はClaudeが進めます。Claude Codeプラグインとして提供されるため、Claudeの手順を固定せずに、複数のリポジトリへ同じ統制を適用できます。
...
成果物を生成しただけでは、ワークフローは次へ進みません。採用する設計を左右する前提は承認前に検証し、範囲を絞った動作確認は最小限の検証手段となる場合にだけ実行します。
...
- 最終レビューで、実装全体が合意した成果を満たし、不要な機能や変更、動作・信頼性・セキュリティの重大な問題がないことを確認したか。
...
| **code-reviewer** | 完成した実装が合意した成果とリポジトリの基準を満たすか確認 |
...
ワークフローがユーザーに判断を求めるのは、要求された成果と対象外を両立できなくなった場合か、元に戻せない外部操作に承認が必要な場合だけです。技術設計、契約、UI、アーキテクチャ、永続化、実装の変更は、プロダクトが提供する価値を変えない限りClaudeが進めます。
