...
- **filesModified** (optional): List of file paths that the upstream agent (typically task-executor-frontend) modified for the current task. Used as the primary scope for Step 1 incomplete-implementation check. When absent, Step 1 falls back to `git diff HEAD`.
...
- **Primary scope**: When the orchestrator passes `filesModified` (the task's write set, typically taken from `task-executor-frontend` response), use only those files.
